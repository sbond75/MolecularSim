# Based on https://github.com/NixOS/nixpkgs/blob/master/pkgs/build-support/fetchurl/default.nix from commit https://github.com/NixOS/nixpkgs/commit/f002ffed9ab96ed074aba0f20c54e813e604fc4e
source $stdenv/setup
...
if [ -n "$downloadToDirectory" ]; then
    downloadedFile="$out"
    if [ -n "$downloadToTemp" ]; then downloadedFile="$TMPDIR/file"; fi
else
    downloadedFile="$out/$downloadToDirectory"
    mkdir -p "$downloadedFile"
fi
